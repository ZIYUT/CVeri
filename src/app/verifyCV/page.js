...
export default function VerifyResume() {
  ...
  const connectWallet = async () => {
    try {
      ...
      setMessage(
        `Wallet connected: ${account}` + (certifierStatus ? ' (Authorized Certifier)' : '')
      );
    } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10 sm:px-6 lg:px-8">
      <div className="text-center mb-10">
        ...
        <p className="mt-2 text-lg">Retrieve and verify professional experience securely</p>
      </div>
            ...
            <div className="mt-8 space-y-6">
              <div>
                ...
                <div className="bg-gray-900 text-white p-4 rounded border border-gray-700 space-y-1">
                  {experienceData.name && (
                    <p>
                      <strong>Name:</strong> {experienceData.name}
                    </p>
                  )}
                  {experienceData.organization && (
                    <p>
                      <strong>Organization:</strong> {experienceData.organization}
                    </p>
                  )}
                  {experienceData.title && (
                    <p>
                      <strong>Title:</strong> {experienceData.title}
                    </p>
                  )}
                  {experienceData.time && (
                    <p>
                      <strong>Time Period:</strong> {experienceData.time}
                    </p>
                  )}
